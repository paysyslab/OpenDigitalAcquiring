...
# Bulk Merchant Profile Updation
...
| **Requester Action(s)** | **Process Description** |
| : | : |
| **Backoffice** | • Backoffice submits a batch file containing merchant profile updates, including a fee value. <br /> • Backoffice displays the rejection message. |
| **Backoffice Services** | • Backoffice Services picks the row with the update parameters. <br /> • Backoffice Services authenticates the request and forwards it to Acquiring Services (RAAST) with the token and client ID. <br /> • Backoffice Services sends the UpdateMerchantProfile request, including MerchantID, Fee, and other parameters, to Acquiring Services (RAAST). |
| **Acquiring Services (RAAST) & RAAST CAS Services** | • Acquiring Services (RAAST) validates the request and responds to Backoffice Services confirming successful authentication. <br /> • Acquiring Services (RAAST) performs business logic validation: <br /> • If the Fee exceeds the maximum threshold, it rejects the request and sends a failure response to Backoffice Services. <br /> • Backoffice Services forwards the rejection to Backoffice. |
